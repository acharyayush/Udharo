...
const UdharoCard = ({
  ...
  const renderCustomerImage = () => {
    ...
    return <BiUserCircle className="text-7xl text-brightGreen" />
  }
  return (
    <div className="UdharoCard relative mt-4 flex min-w-[425px] cursor-pointer items-center rounded-md bg-white p-4 shadow-md duration-200 hover:scale-[1.02]">
      <Link
        to={`udharos/${id}`}
        className="absolute left-0 top-0 h-full w-full"
      ></Link>
      <div>{renderCustomerImage()}</div>
      <div className="udharoDetails text-textColor ml-6 text-left">
      ...
      <div className="actions z-2 absolute bottom-5 right-5">
        <Button
          className={
            "xsm:mr-2 mr-3 bg-red-500 px-3 font-medium duration-200 hover:bg-red-600"
          }
          value={screenSize > 767 && "Delete"}
